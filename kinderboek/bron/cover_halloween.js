...
const front = `<div class="front">${scene}
  <div class="sign"><div class="t1">Cappy's</div><div class="t2">${'Halloween Fun'.split('').map((ch, i) => `<span style="color:${['#f07a1a', '#8a5ad8', '#3fa05a', '#f07a1a', '#8a5ad8', '#3fa05a', '#f07a1a', '#8a5ad8', '#3fa05a', '#f07a1a', '#8a5ad8', '#3fa05a', '#f07a1a'][i]}">${ch === ' ' ? '&nbsp;' : ch}</span>`).join('')}</div>
    <div class="t3">Activity Book for Grade 2</div></div>
...
const back = `<div class="back">
  <h2>A spook-tacular Halloween with a capybara friend!</h2>
  <p>Pick pumpkins, count candy, and go trick-or-treating with Cappy the capybara! This activity book is made for <b>2nd graders (ages 7–8)</b> and mixes coloring with real practice in reading, math, and science.</p>
  ...
  <li>Telling time, counting coins, and skip counting</li><li>Halloween word search, maze, rhymes, and word scramble</li><li>Fall science facts and Halloween coloring pages</li><li>Answer key for parents and teachers</li></ul>
...
.back{width:${0.125 + 8.5}in;height:100%;background:linear-gradient(#f3ecfb,#fde6cf);padding:.75in .75in .75in .875in;position:relative}
.spine{width:${SPINE}in;height:100%;background:#7a5aa6}
...
.back h2{font:700 26pt/1.15 Fredoka;margin:0 0 .2in;color:#6a4a96}
...
(async () => {
  ...
  await p.pdf({ path: path.join(__dirname, 'Cappy_Halloween_Cover_Premium_26p.pdf'), width: W + 'in', height: H + 'in', printBackground: true });
...
process.on('beforeExit', async function fix() {
  ...
  const f = path.join(__dirname, 'Cappy_Halloween_Cover_Premium_26p.pdf');
