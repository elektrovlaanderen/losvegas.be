...
const YEAR = 2026;
const DOTS = ['#f7c08a', '#d7c4ef', '#b9dfc2', '#f6dd8f', '#f2b8bf', '#c9d7f2'];
const TEAL = '#7a5aa6';
...
pages.push(`<div class="title">
  <div class="tt1">Cappy's</div><div class="tt2">Halloween Fun</div>
...
// ---------- 1 Meet Cappy ----------
activity('Meet Pumpkin-Patch Cappy', 'Color Cappy in the pumpkin patch. Then answer the questions.',
  `<div class="pic" style="height:3.9in">${A(4)}</div>
   ${['What costume would you give Cappy?', 'What do you see in the pumpkin patch?', 'What is your favorite Halloween treat?', 'What would you do with Cappy on Halloween?']
    .map((q, i) => `<div class="q">${qnum(i)}<span>${q}</span></div>${lines(1)}`).join('')}`,
  'Answers will vary.');


// ---------- 2 All About Capybaras ----------
const passage = 'It is Halloween! Cappy the capybara wants to join the fun. First, he picks a big orange pumpkin from the patch. Then his mom helps him carve a happy face on it. At night, Cappy puts on his pumpkin costume. He walks with his sister Lulu from house to house. Their neighbors give them apples and treats. Cappy says “thank you” at every door.';
const readQ = [['What does Cappy pick from the patch?', 'A big orange pumpkin.'], ['Who helps Cappy carve the pumpkin?', 'His mom.'],
  ['What costume does Cappy wear?', 'A pumpkin costume.'], ['Who walks with Cappy from house to house?', 'His sister Lulu.'],
  ['What does Cappy say at every door?', '“Thank you.”']];
activity('Cappy’s Halloween Night', 'Read the story. Then answer the questions.',
  `<div class="box read">${passage}</div>
   <div class="pic" style="height:1.55in">${A(2)}</div>
   ${readQ.map(([q], i) => `<div class="q">${qnum(i)}<span>${q}</span></div>${lines(1, 'tight')}`).join('')}`,
  readQ.map(([, a], i) => `${i + 1}. ${a}`).join('<br>'));

// ---------- 3 Color by Number ----------
const key = [['brown', '#c9956a'], ['orange', '#f5a55a'], ['green', '#a9d6a0'], ['yellow', '#f6dc7a'], ['purple', '#c3a6e6'], ['gray', '#cfd3d6']];
activity('Color by Number', 'Use the color code to color the picture.',
  ...
  '1 = Cappy, 2 = pumpkins, 3 = grass, leaves and stems, 4 = moon and stars, 5 = witch hat and bat, 6 = fence and nose.');
...
const add = [[28, 16], [37, 45], [19, 34], [46, 27], [58, 35], [63, 28]];
activity('Candy Count Addition', 'Add the numbers. Write the answer in the box. Then color the pictures.',
  mathGrid(add, '+', poses), add.map(([a, b]) => `${a} + ${b} = <strong>${a + b}</strong>`).join(' · '));
const sub = [[62, 18], [80, 36], [73, 45], [91, 57], [54, 29], [45, 17]];
activity('Spooky Subtraction', 'Subtract the numbers. Write the answer in the box. Then color the pictures.',
  mathGrid(sub, '−', [10, 7, 2, 8, 3, 9].map(A)), sub.map(([a, b]) => `${a} − ${b} = <strong>${a - b}</strong>`).join(' · '));

// ---------- 6 Word Search ----------
const words = ['PUMPKIN', 'CANDY', 'COSTUME', 'BAT', 'MOON', 'GHOST', 'TREAT', 'WITCH', 'OWL', 'LEAVES'];
const N = 11;
...
activity('Halloween Word Search', 'Find and circle the words. They go across → and down ↓.',
...
activity('Help Cappy Find the Pumpkin', 'Draw a line from START to the PUMPKIN. Don’t cross any walls!',
  `<div class="mz"><div class="mzlab">START ↓</div><div class="mzsvg">${mazeSvg(false)}</div><div class="mzlab end">↓ PUMPKIN</div>
   <div class="mzcap">${A(9)}</div><div class="mzriv">${A(7)}</div></div>`,
  `<div style="width:2.4in;height:2.6in">${mazeSvg(true)}</div>`);

// ---------- 8 True or False ----------
const tf = [['Pumpkins grow on vines.', 'T'], ['A pumpkin is a kind of fruit.', 'T'],
  ['Bats are birds.', 'F'], ['Most bats come out at night.', 'T'],
  ['Owls can turn their heads very far around.', 'T'], ['The moon makes its own light.', 'F'],
  ['Many leaves change color in the fall.', 'T'], ['Spiders have six legs.', 'F']];
activity('Halloween Science: True or False?', 'Read each sentence. Circle T for true or F for false.',
  `<div class="tf">${tf.map(([s], i) => `<div class="tfr">${qnum(i)}<span>${s}</span><b>T</b><b>F</b></div>`).join('')}</div>
   <div class="box fact"><b>Did you know?</b> A pumpkin is mostly water. About 9 out of every 10 parts of a pumpkin are water!</div>
   <div class="pic" style="height:2in">${A(2)}</div>`,
  tf.map(([, a], i) => `${i + 1}. ${a === 'T' ? 'True' : 'False'}`).join(' · '));

// ---------- 9 Label Cappy ----------
const parts = [[0, 0, 'hat'], [0, 0, 'eye'], [0, 0, 'nose'], [0, 0, 'whiskers'], [0, 0, 'paw'], [0, 0, 'cape']];
activity('Label Witch Cappy', 'Use the word bank. Write the name of each part next to its number.',
  `<div class="bank">${['whiskers', 'cape', 'hat', 'paw', 'nose', 'eye'].map(w => `<span>${w}</span>`).join('')}</div>
...
// ---------- 10 Sequencing ----------
const seq = [['Cappy and Mom carve a happy face on the pumpkin.', 2], ['Cappy picks a pumpkin from the patch.', 1],
  ['Cappy eats one treat and goes to bed.', 4], ['Cappy puts on his costume and goes trick-or-treating.', 3]];
activity('Cappy’s Halloween Day', 'What happened first? Write 1, 2, 3, or 4 in each box to put the day in order.',
  `<div class="seq">${seq.map(([s]) => `<div class="seqr"><div class="abox sm"></div><span>${s}</span></div>`).join('')}</div>
   <p class="instr" style="margin-top:.25in">Now draw your favorite part of Halloween.</p>
...
const blanks = [2, 4, 6];
activity('Skip Counting on Pumpkins', 'Count by 2s, 5s, 10s, and 100s. Write the missing numbers on the pumpkins.',
...
// ---------- 12 Compare ----------
const cmp = [[54, 45], [27, 72], [66, 66], [100, 99], [18, 81], [305, 350], [421, 412], [79, 97]];
...
// ---------- 13 Place Value ----------
const pv = [462, 735, 908, 250, 613, 187];
activity('Pumpkin Place Value', 'Write how many hundreds, tens, and ones are in each number.',
...
// ---------- 14 Clocks ----------
const times = [[6, 0], [5, 30], [8, 15], [7, 45], [9, 10], [4, 50]];
function clock(h, m) {
...
const coinRows = [[25, 25, 5, 1], [10, 5, 5, 1, 1, 1], [25, 10, 10, 10], [10, 10, 5, 5, 1], [25, 25, 25, 1, 1]];
activity('Trick-or-Treat Coins', 'Add up the coins in each row. Write the total. Color the pennies brown!',
...
// ---------- 16 Story problems ----------
const story = [['Cappy got 23 treats on one street and 19 treats on the next street. How many treats did he get in all?', '23 + 19 = 42 treats'],
  ['There were 32 pumpkins in the patch. The farmers picked 15. How many pumpkins are left?', '32 − 15 = 17 pumpkins'],
  ['Lulu saw 4 bats. Each bat had 2 wings. How many wings did Lulu see?', '2 + 2 + 2 + 2 = 8 wings'],
  ['Cappy had 50 Halloween stickers. He gave 24 to his friends. How many stickers does he have now?', '50 − 24 = 26 stickers']];
activity('Halloween Story Problems', 'Read each problem. Show your work in the box. Write the answer.',
...
// ---------- 17 Rhymes ----------
const rl = ['cat', 'moon', 'night', 'treat', 'boo', 'witch'], rr = ['kite', 'shoe', 'hat', 'switch', 'feet', 'spoon'];
const rMatch = { cat: 'hat', moon: 'spoon', night: 'kite', treat: 'feet', boo: 'shoe', witch: 'switch' };
activity('Rhyme Time', 'Draw a line to match the words that rhyme.',
  `<div class="rhyme"><div>${rl.map(w => `<span>${w}<i></i></span>`).join('')}</div><div>${rr.map(w => `<span><i></i>${w}</span>`).join('')}</div></div>
   <p class="instr">Write two more words that rhyme with <b>bat</b>:</p><div class="grid2">${lines(2)}</div>
   <div class="pic" style="height:2.4in">${A(10)}</div>`,
  rl.map(w => `${w} – ${rMatch[w]}`).join(' · ') + '<br>Rhymes with bat: answers will vary (for example: mat, sat, rat, pat).');

// ---------- 18 Scramble ----------
const scr = [['KNUPMIP', 'PUMPKIN', 'Orange and round'], ['YCNDA', 'CANDY', 'A sweet treat'], ['OTHSG', 'GHOST', 'It says “Boo!”'],
  ['OMON', 'MOON', 'It shines at night'], ['ATB', 'BAT', 'It flies at night'], ['TCIWH', 'WITCH', 'She wears a pointy hat']];
...
const fib = [['Cappy picks a big orange ___.', 'pumpkin'], ['Mom helps Cappy ___ a happy face.', 'carve'], ['Cappy wears a pumpkin ___.', 'costume'],
  ['The ___ is big and round tonight.', 'moon'], ['Two little ___ fly in the sky.', 'bats'], ['The neighbors give Cappy yummy ___.', 'treats']];
activity('Fill in the Blank', 'Choose a word from the word bank to finish each sentence.',
  `<div class="bank">${['moon', 'carve', 'treats', 'pumpkin', 'bats', 'costume'].map(w => `<span>${w}</span>`).join('')}</div>
...
// ---------- 20 Draw & Write ----------
activity('Design a Costume!', 'What costume should Cappy wear next Halloween? Draw it in the box. Then write two sentences about it.',
  `<div class="drawbox" style="height:5.2in"></div>${lines(4)}`, 'Answers will vary.');

// ---------- 21 Coloring page ----------
activity('Coloring Time!', 'Color Cappy and Lulu going trick-or-treating.',
...
pages.push(`<div class="cert"><div class="cin">
  ...
  <p>for finishing <b>Cappy’s Halloween Fun Activity Book</b>!<br>You are a spook-tacular reader, counter, and thinker.</p>
...
.pad{width:.8in;height:.7in;border-radius:50% 50% 50% 50%/60% 60% 40% 40%;border:3px solid #ee9a4a;background:#fdf0e2;display:flex;align-items:center;justify-content:center;font:600 16pt Fredoka}
...
(async () => {
  ...
  await p.pdf({ path: path.join(__dirname, 'Cappy_Halloween_Interior_8.5x11.pdf'), width: '8.5in', height: '11in', printBackground: true, preferCSSPageSize: true });
